...
private:
    ...
    void init_population_chromosome_one_path(unsigned path_count);
...
public:
    ...
    unsigned getBestEdgeModules(int i, int j) {return (unsigned)std::ceil((double)best_edges[i][j] / (modularity * 100));};

    unsigned getBestEdgeFilling(int i, int j) {return (unsigned)std::ceil((double)(best_edges[i][j]%(modularity * 100)/100));};
    ...
    Chromosome startFullDezagregation();
    Chromosome startFullAgregation();
};